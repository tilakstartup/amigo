...
struct ConversationalOnboardingView: View {
    @StateObject private var viewModel: ConversationalOnboardingViewModel
    let onComplete: () -> Void
    
    init(sessionManager: SessionManager, onComplete: @escaping () -> Void) {
        self.onComplete = onComplete
        _viewModel = StateObject(wrappedValue: ConversationalOnboardingViewModel(sessionManager: sessionManager))
    }
            ...
            VStack(alignment: message.isFromAmigo ? .leading : .trailing, spacing: 8) {
                ...
                // Regular message with optional feature card
                    ...
                    if let replyType = message.replyType {
                        ...
                        case "yes_no":
                            YesNoButtonsView(onSelect: onQuickReply, isDisabled: message.isDisabled)
                        case "list":
                            if let replies = message.replies, !replies.isEmpty {
...
#Preview {
    ...
    return ConversationalOnboardingView(sessionManager: sessionManager, onComplete: {})
}
